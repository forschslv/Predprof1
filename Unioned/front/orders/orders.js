...
async function loadDishesForDayDisplay(dayIndex, dishIds) {
    try {
        ...
        constdayContainer = document.getElementById(`day-${dayIndex}-dishes`);
...
<button onclick="adjustQuantity(${dayIndex}, ${dish.id}, -1)">-</button>
        ...
        // Initialize day incurrent orderif (!window.currentOrder.days[dayIndex]) {
...
if (quantity > 0) {
                    ...
                    .catch(error => {
                        console.error(`Error fetchingdish ${dishId}:`, error);
                        return 0;
                    });
...
//Enhancedfunction to loadorders with proper error handling
async function loadMyOrders() {
    try {
        ...
        if (response.ok) {
            ...
            if (orders.length === 0) {
                if(ordersContainer) ordersContainer.innerHTML = '<p>У вас нет заказов</p>';
                return;
            }
...
if (ordersContainer) ordersContainer.innerHTML = ordersHtml;
        ...
        console.error("Errorloading orders:", error);
        constordersContainer = document.getElementById('myOrders');
        if (ordersContainer) ordersContainer.innerHTML = '<p>Ошибка загрузки заказов</p>';
    }
}
